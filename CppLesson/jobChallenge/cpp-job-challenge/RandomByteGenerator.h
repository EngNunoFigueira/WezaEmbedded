#ifndef RANDOMBYTEGENERATOR_H
#define RANDOMBYTEGENERATOR_H

#include <iostream>
#include <cstdio>
#include <cstdlib> 
#include <ctime> 
#include <thread>
#include <string> 
#include <time.h>  
#include <iomanip>
#include <chrono>
#include <mutex> 
#include <algorithm>
#include <memory>
#include <vector>
#include <type_traits>
#include <stdio.h>

#include"RandomByteGenerator.h"

/*----------------------------------------------------------------------------------------------------------------------------------------------------*
* Class RandomByteGenerator, was created to encapsulate the code used to generate the process of Random bytes and get the UTC of each delivered array.*
* As the process is working with data random generated, we decide create a const DATA TESTING to rapidly testing the code...                          * 
* CODE COMMENT                                                                                                                                        *
* We know that the process of creation a random bytes are not easy, because it has some depences, we just decide create our own method...That works!  *
* The first step is generate a random unsigned integer, less than 256 and great than 0. And return the corresponding value stored in our MAP  of char *
*-----------------------------------------------------------------------------------------------------------------------------------------------------*/
class RandomByteGenerator
{
   public:
   RandomByteGenerator();
  ~RandomByteGenerator();
   int RandomGenerator();
   const unsigned char ByteGeneratorUsingIntegerReference();
   const unsigned char getRandomByte();
   std::string getTimeUTC(); 
   unsigned char TestData(int ArrayOrder, const int byteIndex);
   private:
   const unsigned char CHAR_BYTE_MAP [256] {
   
  };
	
};

#endif	